...
description: >
  ...
  Barlow Condensed in lowercase weight 900 treated as graphic primitive, IBM Plex Mono chrome (uppercase,
...
colors:
  # This video runs entirely in the dark register, so the video-wide ground
  # (painted onto the index #root by the assembler) must be ink, not the
  # preset's light canvas — otherwise every crossfade reveals a cream base
  # and washes the cut out to grey.
  canvas: "#080B0F"
  ink-black: "#080B0F"
  ink-black-alt: "#1A1918"
  fire-orange: "#D98324"
  gold: "#E8A23D"
  cream: "#F2EEE6"
  cream-muted: "#C6CCD5"
...
typography:
  ...
  caption: { fontFamily: "Barlow", cqw: 0.9, weight: 500, lineHeight: 1.5 }
  label:   { fontFamily: "IBM Plex Mono", cqw: 0.72, weight: 500, tracking: "0.14em", upper: true }
  # — display / hero ramp (Barlow Condensed, lowercase, negative tracking) —
  h3:      { fontFamily: "Barlow Condensed", cqw: 2.8, weight: 600, lineHeight: 1.2, lower: true }
  quote-text:{ fontFamily: "Barlow Condensed", cqw: 3.8, weight: 700, lineHeight: 1.15, tracking: "-0.02em", lower: true }
  h2:      { fontFamily: "Barlow Condensed", cqw: 4.5, weight: 700, lineHeight: 1.1, tracking: "-0.02em", lower: true }
  stat-value:{ fontFamily: "Barlow Condensed", cqw: 5.5, weight: 900, lineHeight: 1.0, tracking: "-0.04em" }
  h1:      { fontFamily: "Barlow Condensed", cqw: 7.5, weight: 800, lineHeight: 0.9, tracking: "-0.03em", lower: true }
  fadelist-item:{ fontFamily: "Barlow Condensed", cqw: 7.5, weight: 900, lineHeight: 1.0, tracking: "-0.03em", lower: true }
  quote-mark:{ fontFamily: "Barlow Condensed", cqw: 10.0, weight: 900, lineHeight: 0.6 }
  fadelist-title:{ fontFamily: "Barlow Condensed", cqw: 10.5, weight: 900, lineHeight: 0.9, tracking: "-0.04em", lower: true }
  display: { fontFamily: "Barlow Condensed", cqw: 13.0, weight: 900, lineHeight: 0.88, tracking: "-0.04em", lower: true }
...
- **Fonts** — already set to **Barlow Condensed** (display, lowercase 700–900) / **Barlow** (body, regular 400–500); ignore any preset font name lingering in prose.
- **Colors** — use the frontmatter hex; preset color NAMES in prose (e.g. "cobalt", "cream") mean the remapped brand values.

### Sanctioned deviations from the preset (this project only)

1. **Gold `{colors.gold}` #E8A23D is a legitimate SECOND accent.** The preset prose insists
   "fire-orange is the only color" — that rule is **overridden here** by the client's stated brand
   (ember-orange + gold). Use them as a **heat ramp, not two flat accents**: gold is the hotter,
   inner value (spark cores, the white-gold centre of the ember burst, the dawn wash), ember
   `{colors.fire-orange}` is the outer/cooler value (rules, kickers, dividers, spark tips). Never
   place gold and ember as two equal-weight UI colors side by side — that reads corporate. They are
   two temperatures of one fire.
2. **Register discipline still holds.** One register per frame. The **orange register is reserved
   for a single frame** — the 9s chain-break payoff. Frames 01–03 are dark/documentary; the break
   is the only full-environment declaration; frames 05–06 return to dark with a warm gold wash.
   Spending the orange register early kills the turn.
3. **Low-key cinematic lighting is additive.** The preset's plane is flat and shadowless. Here,
   frames may carry a **single soft radial light source** (phone glow, ember core, dawn window) as
   an atmospheric ground. This is lighting, not elevation — still no drop-shadows on components,
   still sharp corners, still 1px hairlines.
4. **Captions are burned in and verbatim.** Caption copy is fixed client copy (see
   `capture/extracted/visible-text.txt`); it may be re-weighted or split across lines for rhythm,
   but **never reworded**.
...
text and becomes graphic primitive.** Barlow Condensed `display` at 13cqw puts a single lowercase word
...
**Barlow Condensed** carries every text role from display to body — expressive range from weight (400–900)
...
- **Massive lowercase Barlow Condensed 900**, negative-tracked, as graphic primitive (display 13cqw).
...
Two ramps. The **reading ramp** (Barlow Condensed body 1.2cqw, mono label 0.72cqw) carries copy + chrome; the
**display ramp** (Barlow Condensed `h2` 4.5cqw → `display` 13cqw, weight 700–900) carries every statement.
...
- **Barlow Condensed display is lowercase, weight 700–900, negative-tracked** (−0.04em largest, −0.02em h2). **Mono chrome is uppercase, 0.1em+.** No italic, no underline, no uppercase display.
...
Barlow Condensed `display` (13cqw) lowercase in ink, left-anchored, over a small ink rule stub + mono kicker; a
Barlow Condensed lead line beneath in 75% ink. **Chrome** mono catalogue number top-left, mono meta top-right
...
**Ground** ink-black. **Composes** kicker, display. **Focal** a 2–4 word Barlow Condensed `display`/`h1`
...
row of three top-border-only stat-cards — big Barlow Condensed-900 numeral in `{colors.fire-orange}`, Barlow Condensed
...
**Ground** ink-black. **Composes** fadelist (3 stacked Barlow Condensed-900 words at 1.0/0.5/0.22), fadelist-title.
...
Barlow Condensed `quote-text` (700, lowercase) at ≤78cqw under an oversized fire-orange `quote-mark` (10cqw,
...
- Set every Barlow Condensed display in **lowercase weight 900**, negative-tracked — the system's signature.
...
- Never uppercase Barlow Condensed display; never add a second accent color.
...
- No serif companion; chrome is never Barlow Condensed.
...
- **Type** — Barlow Condensed lowercase 900 negative-tracked, fit-to-measure; mono chrome uppercase 0.14em; ≥1.4cqw floor.
...
- **Barlow Condensed + IBM Plex Mono via Google Fonts**; Noto Sans SC is the CJK fallback (the lowercase-display signal has no CJK equivalent — the two-register color system carries the identity, per the source).
